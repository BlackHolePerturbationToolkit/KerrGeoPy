    ...
    def test_random(self):
        ...
        for i, params in enumerate(values):
            with self.subTest(i=i,params=params):
                ...
                python_freq_output = np.array(mino_frequencies(a,constants,radial_roots,polar_roots))
                self.assertTrue(np.allclose(abs(mathematica_freq_output[i]),abs(python_freq_output)))
